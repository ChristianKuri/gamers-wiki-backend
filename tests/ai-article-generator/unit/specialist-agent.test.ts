...
// NOTE: title, excerpt, description, tags are now generated by Metadata Agent
// and are no longer part of the plan.
...
describe('runSpecialist', () => {
  ...
  describe('basic functionality', () => {
    ...
    it('generates markdown with section headings (no H1 title)', async () => {
      // NOTE: H1 title is NOT included by Specialist - it's added later by
      // generate-game-article.ts after the Metadata Agent generates the SEO title
      const mockGenerateText = vi.fn().mockResolvedValue({
      ...
      // Should NOT contain H1 title (added after Metadata Agent runs)
      expect(result.markdown).not.toMatch(/^#\s+[^\n]/);
      // Should contain H2 section headings
