/**
 * Franchise Lifecycle Protection Tests
 * 
...
import { shouldProcessFranchiseEvent } from '../../../src/api/franchise/services/franchise-description';

describe('Franchise Lifecycle Protection', () => {
  describe('shouldProcessFranchiseEvent', () => {
    it('should return true for English locale', () => {
      expect(shouldProcessFranchiseEvent('en', 'en')).toBe(true);
    ...
    it('should return false for non-English locales', () => {
      ...
      expect(shouldProcessFranchiseEvent('fr', 'fr')).toBe(false);
      expect(shouldProcessFranchiseEvent(undefined, 'es')).toBe(false);
    });
  });
  ...
  describe('Fire-and-forget pattern for parallel processing', () => {
    ...
    it('should not block on async operations', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        const promise = new Promise<string>((resolve) => {
          setTimeout(() => resolve(`Franchise ${i} description`), 10);
        });
    ...
    it('should allow multiple operations to fail independently', async () => {
      const results: { franchise: string; success: boolean; error?: string }[] = [];

      const operations = [
        Promise.resolve('Description 1')
          .then((desc) => results.push({ franchise: 'Franchise1', success: true })),
        Promise.reject(new Error('Failed'))
          .catch((e: Error) => results.push({ franchise: 'Franchise2', success: false, error: e.message })),
        Promise.resolve('Description 3')
          .then((desc) => results.push({ franchise: 'Franchise3', success: true })),
      ];
  ...
  describe('Strapi 5 publish() behavior assumption', () => {
    ...
    it('documents the assumption that publish() can trigger lifecycle hooks', () => {
      ...
      const file = 'src/api/franchise/content-types/franchise/lifecycles.ts';
  ...
  describe('Franchise-specific considerations', () => {
    /**
     * Franchise-specific tests that differ from companies.
     * Franchises are created less frequently than companies during imports
     * (typically 0-2 per game), but parallel processing is still important.
     */

    it('should handle rapid successive franchise creations', async () => {
      // Simulate rapid franchise creations during a game import
      const franchiseCreations = 3;
      const results: string[] = [];

      const promises = Array.from({ length: franchiseCreations }, (_, i) =>
        Promise.resolve(`Franchise ${i}`)
          .then((name) => {
            results.push(name);
            return name;
          })
      ...
      expect(allResults).toHaveLength(franchiseCreations);
      expect(results).toHaveLength(franchiseCreations);
    });
    ...
    it('should not affect import flow if AI generation fails', async () => {
      ...
      const importFlow = async () => {
        // 1. Create franchise (lifecycle triggers AI generation)
