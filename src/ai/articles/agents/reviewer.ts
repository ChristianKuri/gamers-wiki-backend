...
export async function runReviewer(
  ...
  // Extract title from markdown H1 (first line: # Title)
  // NOTE: Title is now generated by Metadata Agent and prepended to markdown,
  // not part of the plan. We extract it here for logging and prompt context.
  const titleMatch = markdown.match(/^#\s+(.+)$/m);
  const extractedTitle = titleMatch ? titleMatch[1].trim() : plan.gameName;

  log.info(`Starting review for article: "${extractedTitle}"`);
  ...
  const researchSummary = buildResearchSummaryForReviewer(
    scoutOutput.sourceSummaries,
    REVIEWER_CONFIG.MAX_RESEARCH_CONTEXT_LENGTH
  );

  // Create a plan-like object with the extracted title for prompt context
  // This maintains backwards compatibility with prompts that expect plan.title
  const planWithTitle = {
    ...plan,
